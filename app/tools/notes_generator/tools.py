...
class AINotesGenerator:
    def __init__(self, args=None, vectorstore_class=Chroma, prompt=None, embedding_model=None, parser=None, model=None, verbose=False):
        default_config = {
            "model": GoogleGenerativeAI(model="gemini-1.5-flash"),
            "embedding_model": GoogleGenerativeAIEmbeddings(model='models/embedding-001'),
            "parser": JsonOutputParser(), # TODO: Add parser
            "prompt": read_text_file("prompt/notes-prompt.txt"),
            "prompt_without_topic": read_text_file("prompt/notes-without-topic-prompt.txt"),
        ...
        if args.page_layout is None: raise ValueError("Page Layout must be provided")
    ...
    def generate_notes(self, documents: Optional[List[Document]]):
        ...
        # TODO: Make sure to include the direct text input
        if (documents):
            chain = self.compile_with_docs(documents)
        else:
            chain = self.compile_without_docs()
        
        response = chain.invoke(f"""Notes Topic: {self.args.topic}, 
                                Page Layout: {self.args.page_layout}, 
